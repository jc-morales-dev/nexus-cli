...
import { getNexusApiKeyFromEnv } from './env'
import { run } from './run'

import type { RunOptions, NexusClientOptions } from './run'
import type { RunState } from './run-state'

export class NexusClient {
  public options: NexusClientOptions & {
    apiKey: string
    fingerprintId: string
  }

  constructor(options: NexusClientOptions) {
    const foundApiKey = options.apiKey ?? getNexusApiKeyFromEnv()
    if (!foundApiKey) {
      throw new Error(
        `Nexus API key not found. Please provide an apiKey in the constructor of NexusClient or set the ${API_KEY_ENV_VAR} environment variable.`,
      )
    }
  ...
  /**
   * Run a Nexus agent with the specified options.
   *
   ...
   * @param projectFiles - (Optional) All the files in your project as a plain JavaScript object. Keys should be the full path from your current directory to each file, and values should be the string contents of the file. Example: { "src/index.ts": "console.log('hi')" }. This helps Nexus pick good source files for context.
   ...
   */
  public async run(
    options: RunOptions & NexusClientOptions,
  ): Promise<RunState> {
    return run({ ...this.options, ...options })
  }

  /**
   * Check connection to the Nexus backend by hitting the /healthz endpoint.
   *
   * @returns Promise that resolves to true if connected, false otherwise
   */
  public async checkConnection(): Promise<boolean> {
    // BYOK direct mode talks to the provider directly; there is no Nexus
