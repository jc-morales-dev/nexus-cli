...
function runCLI(
  ...
  const result = spawnSync(BUN_EXECUTABLE, ['run', CLI_PATH, ...args], {
...
describe.skipIf(!sdkBuilt)('CLI End-to-End Tests', () => {
  ...
  test(
    ...
    async () => {
      ...
      const proc = spawn(BUN_EXECUTABLE, ['run', CLI_PATH, '--agent', 'ask'], {
        cwd: path.join(__dirname, '../..'),
        stdio: 'pipe',
      })
  ...
  test(
    ...
    async () => {
      const proc = spawn(BUN_EXECUTABLE, ['run', CLI_PATH, '--clear-logs'], {
        cwd: path.join(__dirname, '../..'),
        stdio: 'pipe',
      })
