...
- **Works with any TUI app** - Nexus, Claude Code, Codex, or any custom TUI application
...
### Testing Nexus (Default)

```bash
# Start a test session (defaults to Nexus dev server)
...
```bash
# Build the binary first (Nexus example)
cd cli && bun run build:binary

# Test with binary at default location (./cli/bin/nexus)
...
NEXUS_BINARY=./cli/bin/nexus ./scripts/tmux/tmux-cli.sh start
```
...
# Default settings (Nexus dev server)
...
# Test a compiled binary (default location: ./cli/bin/nexus)
...
- `@nexus-tester` - Tests the Nexus CLI
