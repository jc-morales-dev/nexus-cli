...
const createCodeSearcherBlock = (
  ...
  params: {
    searchQueries: [
      { pattern: 'FREETIER_MODEL_SELECTOR_MODELS' },
      { pattern: 'FREETIER_MODEL_SELECTOR_MODEL_IDS' },
      { pattern: 'DEFAULT_FREETIER_MODEL_ID' },
    ],
  },
  blocks: [],
  ...options,
...
describe('code search summary helpers', () => {
  ...
  test('handles singular labels', () => {
    const agentBlock = createCodeSearcherBlock({
      params: {
        searchQueries: [{ pattern: 'DEFAULT_FREETIER_MODEL_ID' }],
      },
