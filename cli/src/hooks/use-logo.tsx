 ...
 * - Text variant "NEXUS" or "Nexus CLI" for narrow widths
 *
...
export const useLogo = ({
  ...
  const rawLogoString = useMemo(() => {
    if (maxHeight != null && maxHeight < ASCII_LOGO_LINES) {
      return IS_FREETIER ? 'FREETIER' : 'NEXUS'
    }
  ...
  const textBlock = useMemo(() => {
    if (rawLogoString === 'NEXUS' || rawLogoString === 'FREETIER') {
  ...
  const component = useMemo(() => {
    ...
    if (rawLogoString === 'NEXUS' || rawLogoString === 'FREETIER') {
      ...
      // the terminal is narrow), keep it to the bare brand name — "FreeTier
