...
try {
  ...
  console.log('✅ Named destructuring successful:', typeof NexusClient)

  if (typeof NexusClient !== 'function') {
    throw new Error(
      `Expected NexusClient to be a function, got ${typeof NexusClient}`,
    )
  }
  ...
  const ClientFromDestructure = require('@nexus/sdk').NexusClient
  const ClientFromDefault = require('@nexus/sdk').NexusClient
  ...
  try {
    ...
    eval('import { NexusClient } from "@nexus/sdk"')
