...
2. **Test your agent**: Run `nexus` without `--agent` so local `.agents` load, then invoke your agent from a prompt
3. **Publish your agent**: Run `nexus publish your-agent-name`

## Need Help?

- For examples, check the `examples/` directory.
- Join our [Discord community](https://nexus.com/discord) and ask your questions!
- Check our [documentation](https://nexus.com/docs) for more details

# What is Nexus?

Nexus is an **open-source AI coding assistant** that edits your codebase through natural language instructions. Instead of using one model for everything, it coordinates specialized agents that work together to understand your project and make precise changes.

Nexus beats Claude Code at 61% vs 53% on [our evals](https://github.com/NexusAI/nexus/tree/main/evals) across 175+ coding tasks over multiple open-source repos that simulate real-world tasks.
...
This guide covers everything you need to know about building custom Nexus agents.
...
1. **Local Testing**: Run `nexus` without `--agent` so local `.agents` load, then invoke your agent from a prompt
...
3. **Publish**: `nexus publish your-agent-name`
...
## Why Choose Nexus for Custom Agents
...
**Fully customizable SDK**: Build Nexus's capabilities directly into your applications with a complete TypeScript SDK. Create custom tools, integrate with your CI/CD pipeline, build AI-powered development environments, or embed intelligent coding assistance into your products.

Learn more about the SDK [here](https://www.npmjs.com/package/@nexus/sdk).

## Community & Support

- **Discord**: [Join our community](https://nexus.com/discord) for help and inspiration
- **Examples**: Study the `examples/` directory for patterns
- **Documentation**: [nexus.com/docs](https://nexus.com/docs) and check `types/` for detailed type information
- **Issues**: [Report bugs and request features on GitHub](https://github.com/NexusAI/nexus/issues)
