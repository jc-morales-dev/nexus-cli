...
// Shape note: exported function called from the entry point (not a side-effect
// import) — bun build --compile strips side-effect-only modules from the
// compiled binary regardless of the package.json sideEffects list.
import { existsSync, readFileSync } from 'fs'
import { dirname, isAbsolute, join, resolve } from 'path'

export function initTreeSitterWasm(): void {
  ...
  // Also publish on globalThis so the smoke handler in the entry can
...
// the top of main() in cli/src/cli-main.tsx (before parseArgs).
