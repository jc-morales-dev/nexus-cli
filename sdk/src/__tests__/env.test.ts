...
describe('sdk/env', () => {
  describe('getSdkEnv', () => {
    ...
    test('returns current process.env values for NEXUS_RG_PATH', () => {
      process.env.NEXUS_RG_PATH = '/path/to/rg'
      const env = getSdkEnv()
      expect(env.NEXUS_RG_PATH).toBe('/path/to/rg')
    })

    test('returns current process.env values for NEXUS_WASM_DIR', () => {
      process.env.NEXUS_WASM_DIR = '/path/to/wasm'
      const env = getSdkEnv()
      expect(env.NEXUS_WASM_DIR).toBe('/path/to/wasm')
    })
    ...
    test('returns undefined for unset env vars', () => {
      ...
      delete process.env.NEXUS_WASM_DIR
      const env = getSdkEnv()
      expect(env.NEXUS_RG_PATH).toBeUndefined()
      expect(env.NEXUS_WASM_DIR).toBeUndefined()
    })

    test('returns a snapshot that does not change when process.env changes', () => {
      process.env.NEXUS_RG_PATH = '/original/path'
      const env = getSdkEnv()
      process.env.NEXUS_RG_PATH = '/new/path'
      expect(env.NEXUS_RG_PATH).toBe('/original/path')
    })
  })
  ...
  describe('createTestSdkEnv', () => {
    ...
    test('allows overriding SDK-specific values', () => {
      const env = createTestSdkEnv({
        NEXUS_RG_PATH: '/custom/rg',
        NEXUS_WASM_DIR: '/custom/wasm',
      })
      expect(env.NEXUS_RG_PATH).toBe('/custom/rg')
      expect(env.NEXUS_WASM_DIR).toBe('/custom/wasm')
