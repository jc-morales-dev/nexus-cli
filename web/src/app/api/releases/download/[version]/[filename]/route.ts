...
export async function GET(
  ...
  // Freebuff releases use a "freetier-v" tag prefix to avoid colliding with codebuff releases
  const tagPrefix = filename.startsWith('freetier-') ? 'freetier-v' : 'v'

  // Current download location - can be changed in the future without affecting old clients
  const downloadUrl = `https://github.com/CodebuffAI/nexus-community/releases/download/${tagPrefix}${version}/${filename}`
