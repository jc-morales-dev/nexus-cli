...
| Jane | 25  |`
  ...
  test('renders code fence followed by text with quotes correctly', () => {
    const markdown = `\`\`\`bash
# Start using it
nexus "add a new feature to handle user authentication"
\`\`\``
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
  ...
  test('renders inline code followed by quotes correctly', () => {
    const markdown = 'Use `nexus "fix bug"` to fix bugs.'
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
...
\`\`\``
  ...
  test('renders code fence with command and quotes on same line', () => {
    const markdown = `\`\`\`bash
nexus "implement feature" --verbose
\`\`\``
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
    ...
    expect(textContent).toContain('nexus "implement feature" --verbose')
    expect(textContent).not.toContain('```"')
  })
