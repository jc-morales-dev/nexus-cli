...
#   -b, --binary [PATH] Use compiled binary (Nexus-specific shortcut)
#                       If PATH omitted, uses ./cli/bin/nexus
#                       Can also set NEXUS_BINARY env var
...
#
#   # Start with default Nexus dev server (backward compatible)
...
#
#   # Test a compiled binary (Nexus default location)
#   ./scripts/tmux/tmux-start.sh --binary
#
#   # Test a compiled binary at custom path
#   ./scripts/tmux/tmux-start.sh --binary ./path/to/nexus
#
# EXIT CODES:
...
WAIT_SECONDS=4
DEFAULT_BINARY="$PROJECT_ROOT/cli/bin/nexus"
BINARY_PATH="${NEXUS_BINARY:-}"  # Environment variable takes precedence
...
if [[ -n "$CUSTOM_COMMAND" ]]; then
    ...
    # Default mode - Nexus dev server via bun (for backward compatibility)
