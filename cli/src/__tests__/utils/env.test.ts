...
describe('cli/utils/env', () => {
  describe('getCliEnv', () => {
    ...
    test('returns current process.env values for editor preferences', () => {
      ...
      process.env.NEXUS_CLI_EDITOR = 'code'
      const env = getCliEnv()
      expect(env.EDITOR).toBe('vim')
      expect(env.NEXUS_CLI_EDITOR).toBe('code')
    })
    ...
    test('returns current process.env values for binary build config', () => {
      ...
      process.env.NEXUS_CLI_VERSION = '1.0.0'
      const env = getCliEnv()
  ...
  describe('createTestCliEnv', () => {
    ...
    test('allows overriding editor preferences', () => {
      const env = createTestCliEnv({
        EDITOR: 'vim',
        VISUAL: 'code',
        NEXUS_CLI_EDITOR: 'cursor',
      })
      expect(env.EDITOR).toBe('vim')
      expect(env.VISUAL).toBe('code')
      expect(env.NEXUS_CLI_EDITOR).toBe('cursor')
    })

    test('allows overriding binary build config', () => {
      const env = createTestCliEnv({
        NEXUS_IS_BINARY: 'true',
        NEXUS_CLI_VERSION: '2.0.0',
        NEXUS_CLI_TARGET: 'darwin-arm64',
      })
      expect(env.NEXUS_IS_BINARY).toBe('true')
      expect(env.NEXUS_CLI_VERSION).toBe('2.0.0')
      expect(env.NEXUS_CLI_TARGET).toBe('darwin-arm64')
    })
