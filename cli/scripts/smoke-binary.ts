...
//
//   - "will run commands on your behalf" — nexus/freetier main surface
//     header (authed + session ready)
//   - "Press ENTER to login" / "Open this URL" — login modal (no cached
//     creds — typical CI smoke)
//   - "Pick a model to start" / waiting-room copy — freetier queue gate
//   - "Free mode isn't available" — freetier country-block screen (CI
...
// fires after the renderer is up). The wasm-load rejection on freetier
...
// what tripped freetier 0.0.62 in the wild while a 5s window let CI pass.
