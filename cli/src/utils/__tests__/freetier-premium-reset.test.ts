...
describe('freetier premium reset helpers', () => {
  ...
  test('keeps expired server resetAt instead of rolling stale quota forward', () => {
    ...
    expect(resetAt.toISOString()).toBe('2026-05-12T07:00:00.000Z')
    expect(formatFreeTierPremiumResetCountdown(resetAt, nowMs)).toBe('now')
  })
  ...
  test('formats hours and minutes left', () => {
    ...
    expect(formatFreeTierPremiumResetCountdown(resetAt, nowMs)).toBe('11h 30m')
  })
  ...
  test('formats sub-hour reset countdowns', () => {
    ...
    expect(formatFreeTierPremiumResetCountdown(resetAt, nowMs)).toBe('30m')
  })
})
