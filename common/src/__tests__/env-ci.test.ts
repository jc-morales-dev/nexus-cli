...
describe('env-ci', () => {
  describe('getCiEnv', () => {
    ...
    test('returns current process.env values for NEXUS_API_KEY', () => {
      process.env.NEXUS_API_KEY = 'test-key-123'
      const env = getCiEnv()
      expect(env.NEXUS_API_KEY).toBe('test-key-123')
    })

    test('returns current process.env values for NEXUS_GITHUB_TOKEN', () => {
      process.env.NEXUS_GITHUB_TOKEN = 'ghp_test_token'
      const env = getCiEnv()
      expect(env.NEXUS_GITHUB_TOKEN).toBe('ghp_test_token')
    })
  ...
  describe('ciEnv', () => {
    ...
    test('contains expected keys', () => {
      ...
      expect('NEXUS_API_KEY' in ciEnv).toBe(true)
      expect('NEXUS_GITHUB_TOKEN' in ciEnv).toBe(true)
    })
  })
  ...
  describe('createTestCiEnv', () => {
    test('returns a CiEnv with default test values', () => {
      ...
      expect(env.NEXUS_API_KEY).toBe('test-api-key')
    })
    ...
    test('allows overriding default values', () => {
      const env = createTestCiEnv({
        NEXUS_API_KEY: 'custom-api-key',
      })
      expect(env.NEXUS_API_KEY).toBe('custom-api-key')
    })
    ...
    test('allows setting all CI-related vars for CI simulation', () => {
      const env = createTestCiEnv({
        ...
        NEXUS_GITHUB_TOKEN: 'ghp_simulated_token',
        NEXUS_API_KEY: 'test-api-key-override',
      })
      expect(env.CI).toBe('true')
      expect(env.GITHUB_ACTIONS).toBe('true')
      expect(env.RENDER).toBe('true')
      expect(env.IS_PULL_REQUEST).toBe('true')
      expect(env.NEXUS_GITHUB_TOKEN).toBe('ghp_simulated_token')
      expect(env.NEXUS_API_KEY).toBe('test-api-key-override')
    })
  })
})
